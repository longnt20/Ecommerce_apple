...
onMounted(async () => {
  try {
    ...
    console.log(">>> CheckoutPage mounted, mode:", mode);

    // Load provinces - luôn thực hiện đầu tiên
    try {
      ...
      await nextTick();
      console.log(">>> Provinces.value set:", provinces.value?.length);
    } catch (error) {
    ...
    if (mode === "buy-now" && buyNow.active) {
      ...
      console.log(">>> BUY NOW mode completed");
    } else {
      // NORMAL CART MODE
      try {
        ...
        cart.value = res.data;
        console.log(">>> NORMAL CART mode completed");
      } catch (error) {
        console.error(">>> Error loading cart:", error);
      }
    }
    
    console.log(">>> CheckoutPage mounted successfully!");
  } catch (error) {
    console.error(">>> Error in CheckoutPage onMounted:", error);
  }
});
...
async function fetchDistricts() {
  ...
  wards.value = [];
  const provinceId = shipping.value.province_id;
  
  console.log(">>> Selected provinceId:", provinceId);
  
  // Convert string to number if needed
  const provinceCode = parseInt(provinceId) || provinceId;
  console.log(">>> Converted provinceCode:", provinceCode);

  if (!provinceId) {
    console.log(">>> No province selected, returning");
    return;
  }
  ...
  try {
    ...
    console.log(">>> Districts type:", typeof districtsData);
    console.log(">>> Districts length:", districtsData?.length);
    
    districts.value = districtsData;
    console.log(">>> Districts.value set:", districts.value?.length);
  } catch (error) {
...
async function fetchWards() {
  console.log(">>> fetchWards called");
  shipping.value.ward_id = "";
  const districtId = shipping.value.district_id;
  
  console.log(">>> Selected districtId:", districtId);
  
  // Convert string to number if needed
  const districtCode = parseInt(districtId) || districtId;
  console.log(">>> Converted districtCode:", districtCode);

  if (!districtId) {
    console.log(">>> No district selected, returning");
    return;
  }
  ...
  try {
    ...
    console.log(">>> Wards response:", wardsData);
    console.log(">>> Wards type:", typeof wardsData);
    console.log(">>> Wards length:", wardsData?.length);
    
    wards.value = wardsData;
    console.log(">>> Wards.value set:", wards.value?.length);
  } catch (error) {
